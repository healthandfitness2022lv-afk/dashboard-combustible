...
// ---- Chart: Distribución por equipo ----
function renderChartDistribucion() {
  const totals = {};
  _records.forEach(r => {
    totals[r.equipoId] = (totals[r.equipoId] || 0) + (r.litriosIngresados || 0);
  });
  ...
  const colors = [
    '#003478','#1F4E8F','#27AE60','#D4AF37',
    '#F39C12','#E74C3C','#8E44AD','#16A085',
  ];
...
function renderTbodyRecords() {
  ...
  if (!_recordsFiltered.length) {
    tbody.innerHTML = '<tr><td colspan="6" class="td-loading">Sin registros</td></tr>';
    return;
  }
...
function renderRegistrosSection() {
  ...
  const litros  = _records.reduce((s, r) => s + (r.litriosIngresados || 0), 0);
  const prom    = total ? litros / total : 0;
  const ops     = new Set(_records.map(r => r.operador)).size;

  document.getElementById('kpiTotalRegistros').textContent  = total;
  document.getElementById('kpiPromedioLitros').textContent  = fmt(prom, 1);
